...
Daily aggregator of South Dakota-located taildragger classified
...
[South Carolina](https://github.com/taildraggers/south-carolina) repos'
approach, with the state filter swapped to "SD".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, SD` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in South Dakota"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/south-dakota/`
...
```html
<iframe
  src="https://taildraggers.github.io/south-dakota/"
  title="Taildragger Ads in South Dakota"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_SOUTH_DAKOTA_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
