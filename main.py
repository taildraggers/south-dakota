"""Aggregates South Dakota-located listings from every taildraggers.com
...
ones whose location ends in ", SD".
...
Pennsylvania/Rhode Island/South Carolina repos' approach, with the state
filter swapped to "SD".
"""
...
PAGE_TITLE = "Taildragger Ads in South Dakota"
STATE_ABBR = "SD"
...
_SOUTH_DAKOTA_LOCATION_RE = re.compile(r",\s*SD\s*$", re.IGNORECASE)


def _is_south_dakota(location: str) -> bool:
    return bool(_SOUTH_DAKOTA_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        south_dakota_rows = [row for row in rows if _is_south_dakota(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(south_dakota_rows)} in South Dakota")
        all_listings.extend(south_dakota_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No South Dakota listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique South Dakota listings: {len(listings)}")
